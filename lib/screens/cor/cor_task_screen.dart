...
class CorTaskScreen extends StatefulWidget {
  const CorTaskScreen({Key? key}) : super(key: key);
...
class _CorTaskScreenState extends State<CorTaskScreen> {
  ...
  Future<void> _fetchOrders() async {
    ...
    try {
      ...
      setState(() {
        // Tampilkan pesanan yang statusnya readyForCasting atau casting (untuk cor)
        _orders =
            orders
                .where(
                  (o) =>
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Gagal memuat tugas cor: $e')));
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Daftar Tugas Cor'),
        actions: [
          IconButton(
      ...
      body:
          _isLoading
              ...
              : _orders.isEmpty
              ? const Center(child: Text('Tidak ada tugas cor.'))
              : RefreshIndicator(
