...
class _CorDetailScreenState extends State<CorDetailScreen> {
  ...
  final List<String> _casterTasks = [
          ...
          : Padding(
              ...
              child: ListView(
                children: [
                  ...
                  if (isWorking) ...[
                    const Divider(),
                    const Text('Tugas Caster', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ..._casterTasks.map((task) => CheckboxListTile(
                    ...
                    ElevatedButton(
                      onPressed: (_isProcessing ||
                          !_casterTasks.every((task) => _order.castingWorkChecklist.contains(task)))
                          ? null
                          : () async {
                              setState(() => _isProcessing = true);
                              try {
                                ...
                                await _fetchOrderDetail();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Order berhasil disubmit ke Carving')),
                                );
                      ...
                      onPressed: _isProcessing
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                await _fetchOrderDetail();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Pesanan diterima, status menjadi Casting')),
                                );
                                Navigator.of(context).pop(true);
                              } finally {
                                setState(() => _isProcessing = false);
