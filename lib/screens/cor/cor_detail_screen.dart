...
class CorDetailScreen extends StatefulWidget {
  ...
  const CorDetailScreen({super.key, required this.orderId});
...
class _CorDetailScreenState extends State<CorDetailScreen> {
  ...
  Future<void> _fetchOrder() async {
    ...
    setState(() {
      _order = order;
      _checked = List<String>.from(order.castingWorkChecklist ?? []);
      _isLoading = false;
    });
  }
  ...
  Future<void> _updateChecklist(String task, bool checked) async {
    ...
    await OrderService().updateOrder(
      _order!.copyWith(castingWorkChecklist: _checked),
    );
  }

  Future<void> _startCasting() async {
    final success = await OrderService().updateOrder(
      _order!.copyWith(workflowStatus: OrderWorkflowStatus.casting),
    );
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gagal memulai casting!')),
      );
    }
  }
  ...
  Future<void> _submitToCarver() async {
    final success = await OrderService().updateOrder(
      _order!.copyWith(
        ...
        castingWorkChecklist: _checked,
      ),
    );
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pesanan berhasil dikirim ke Carver!')),
      );
  ...
  Widget build(BuildContext context) {
    ...
    final isWaitingCasting = _order?.workflowStatus == OrderWorkflowStatus.waitingCasting;
    final isCasting = _order?.workflowStatus == OrderWorkflowStatus.casting;
    final allChecked = _tasks.every((task) => _checked.contains(task));

    return Scaffold(
      appBar: AppBar(
        title: const Text('Detail Pesanan Cor'),
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            // Checklist Cor
            _buildChecklist('Checklist Cor', _order!.castingWorkChecklist),
            ...
            if (isWaitingCasting)
              ElevatedButton.icon(
                ...
                label: const Text('Mulai Casting!'),
            ...
            if (isCasting) ...[
              const Text('Checklist Tugas Cor', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const Divider(),
              ...
              ElevatedButton.icon(
                ...
                onPressed: allChecked ? _submitToCarver : null,
              ),
            ],
          ],
        ),
