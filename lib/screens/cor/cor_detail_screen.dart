...
class _CorDetailScreenState extends State<CorDetailScreen> {
  ...
  void initState() {
    super.initState();
    _order = widget.order;
    _corChecklist = List<String>.from(_order.ordersCastingWorkChecklist);
  ...
  Future<void> _refreshOrderData() async {
    try {
      ...
      setState(() {
        ...
        _corChecklist = List<String>.from(
          refreshedOrder.ordersCastingWorkChecklist,
        );
      });
    } catch (e) {
  ...
  Future<void> _startCasting() async {
    ...
    try {
      ...
      final Map<String, dynamic> updateFields = {
        'ordersWorkflowStatus': OrderWorkflowStatus.casting,
        'ordersCastingAccountId': currentUserId,
      };
      ...
      final updatedOrder = _order.copyWith(
        ...
        ordersCastingAccountId: updateFields['ordersCastingAccountId'],
      ...
      print(
        'DEBUG: updatedOrder.ordersCastingAccountId = ${updatedOrder.ordersCastingAccountId}',
      );
      ...
      if (result == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Pesanan masuk tahap Casting')),
        );
        // Refresh layar
  ...
  Future<void> _updateChecklist() async {
    ...
    try {
      final updatedOrder = _order.copyWith(
        ordersCastingWorkChecklist: _corChecklist,
      );
      await OrderService().updateOrder(updatedOrder);
      setState(() {
        _order = updatedOrder;
        _corChecklist = List<String>.from(
          updatedOrder.ordersCastingWorkChecklist,
        );
      });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  Future<void> _submitToCarver() async {
    setState(() => _isProcessing = true);
    try {
      final updatedOrder = _order.copyWith(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pesanan dikirim ke Carver!')),
      );
      Navigator.of(context).pop(true);
    } finally {
      setState(() => _isProcessing = false);
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: _isProcessing ? null : _startCasting,
                  ),
                ),
              ),
                ...
                children: [
                  ...corTasks.map(
                    (task) => CheckboxListTile(
                      value: _corChecklist.contains(task),
                      title: Text(task),
                      onChanged: (val) {
                        setState(() {
                          if (val == true) {
                            _corChecklist.add(task);
                          } else {
                            _corChecklist.remove(task);
                          }
                        });
                      },
                    ),
                  ...
                  const SizedBox(height: 12),
                  if (_corChecklist.length == corTasks.length &&
                      _corChecklist.toSet().containsAll(corTasks.toSet()) &&
                      _order.ordersCastingWorkChecklist.length ==
                          corTasks.length &&
                      _order.ordersCastingWorkChecklist.toSet().containsAll(
                        corTasks.toSet(),
                      ))
                    SizedBox(
                      width: double.infinity,
                      child: ElevatedButton.icon(
                        ...
                        onPressed: _isProcessing ? null : _submitToCarver,
                      ),
                    ),
                ],
              ),
