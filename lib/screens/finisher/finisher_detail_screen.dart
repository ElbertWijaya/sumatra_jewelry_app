...
class FinisherDetailScreen extends StatefulWidget {
  ...
  const FinisherDetailScreen({super.key, required this.orderId});
...
class _FinisherDetailScreenState extends State<FinisherDetailScreen> {
  ...
  Future<void> _fetchOrder() async {
    ...
    setState(() {
      _order = order;
      _checked = List<String>.from(order.finishingWorkChecklist ?? []);
      _isLoading = false;
    });
  }
  ...
  Future<void> _updateChecklist(String task, bool checked) async {
    ...
    await OrderService().updateOrder(
      _order!.copyWith(finishingWorkChecklist: _checked),
    );
  }

  Future<void> _startFinishing() async {
    final success = await OrderService().updateOrder(
      _order!.copyWith(workflowStatus: OrderWorkflowStatus.finishing),
    );
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gagal memulai finishing!')),
      );
    }
  }
  ...
  Future<void> _submitToInventory() async {
    final success = await OrderService().updateOrder(
      _order!.copyWith(
        ...
        finishingWorkChecklist: _checked,
      ),
    );
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pesanan berhasil dikirim ke Inventory!')),
      );
  ...
  Widget build(BuildContext context) {
    ...
    final isWaitingFinishing = _order?.workflowStatus == OrderWorkflowStatus.waitingFinishing;
    final isFinishing = _order?.workflowStatus == OrderWorkflowStatus.finishing;
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            if (isWaitingFinishing)
              ElevatedButton.icon(
                icon: const Icon(Icons.play_arrow),
                label: const Text('Mulai Finishing!'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.blue,
                  foregroundColor: Colors.white,
                  minimumSize: const Size(double.infinity, 48),
                ),
                onPressed: _startFinishing,
              ),

            if (isFinishing) ...[
              const Text('Checklist Tugas Finisher', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const Divider(),
              ...
              ElevatedButton.icon(
                ...
                label: const Text('Submit ke Inventory'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: allChecked ? Colors.green : Colors.grey,
                  foregroundColor: Colors.white,
                  minimumSize: const Size(double.infinity, 48),
                ),
                onPressed: allChecked ? _submitToInventory : null,
              ),
            ],
          ],
        ),
