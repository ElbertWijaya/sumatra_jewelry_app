          ...
          : Padding(
              ...
              child: ListView(
                children: [
                  ...
                  if (isWorking) ...[
                    const Divider(),
                    const Text('Tugas Finisher', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ..._finisherTasks.map((task) => CheckboxListTile(
                          ...
                          onChanged: _isProcessing
                              ...
                              : (val) {
                                  setState(() {
                                    if (val == true) {
                                      if (!_finisherChecklist.contains(task)) {
                    ...
                    ElevatedButton(
                      onPressed: (_isProcessing ||
                          !_finisherTasks.every((task) => _order.finishingWorkChecklist.contains(task)))
                          ? null
                          : () async {
                              setState(() => _isProcessing = true);
                              try {
                                ...
                                await _fetchOrderDetail();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Order berhasil disubmit ke Inventory')),
                                );
                      ...
                      onPressed: _isProcessing
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                await _fetchOrderDetail();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Pesanan diterima, status menjadi Finishing')),
                                );
                                Navigator.of(context).pop(true);
                              } finally {
                                setState(() => _isProcessing = false);
